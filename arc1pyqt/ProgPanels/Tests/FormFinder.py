...
class ThreadWrapper(BaseThreadWrapper):
    ...
    def run(self):
        ...
        for device in self.deviceList:
            ...
            #here put the the three functions below
            self.read_pulse(w,b,1)
            
            self.displayData.emit()
            self.updateTree.emit(w,b)


    def read_pulse(self, w, b, cycles):#extracts several resistance readings from the board
        pass                                          #in the same parameterFit external csv log file
                                                  #at the end of each formfit run                                                     
        #resMidTag="%s_%%s_i" % resTag
        ...
        for i in range(cycles):
            ...
            Mnow=f.getFloats(1)
            data_pf = {'Measurement': "READPULSE", 'D': state.Application.Dice_no, 'S': state.Application.Subdice_no, \
                           'W': w, 'B': b, 'Res': Mnow, 'V': g.Vread, \
                           'PW': 0, 'Time': str(time.time())}
            dictwriter.writerow(data_pf)
            
            print(Mnow)
            
    def write_pulse(self, w, b, voltage, pw):
        #write a pulse with specific config
        pass
        
    def compute_formingVoltage(self, input_v):
        #compute the output voltage for the pulse accordingly
        pass
